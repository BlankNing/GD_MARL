...
class MADDPG_Attention:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.model_path=self.model_path+'/share_param='+str(self.args.share_param)
        if not os.path.exists(self.model_path):
            os.mkdir(self.model_path)
    ...
    def train(self, transitions, other_agents):
        ...
        r_s=r[:,-1].squeeze()
        o, u, o_next ,u_next= [], [], [],[]  # 用来装每个agent经验中的各项 seq_version
        o_s,u_s,o_next_s=[],[],[]
        for agent_id in range(self.args.n_agents):
            o.append(transitions['o_%d' % agent_id])
            o_s.append(transitions['o_%d' % agent_id][:,-1,:].squeeze())
            u.append(transitions['u_%d' % agent_id])
            u_next.append(transitions['u_%d' % agent_id][:,1:,:])
            ...
            if not self.args.share_param:
                ...
                for agent_id in range(self.args.n_agents):
                    if agent_id == self.agent_id:
                        u_next_a=self.actor_target_network.forward(o_next[agent_id])
                        u_next_a=u_next_a.unsqueeze(1)
                        u_next[agent_id]=torch.cat((u_next[agent_id],u_next_a),dim=1)
                    else:
                        #
                        u_next_a=other_agents[index].policy.actor_target_network(o_next[agent_id])
                        u_next_a = u_next_a.unsqueeze(1)
                        u_next[agent_id] = torch.cat((u_next[agent_id], u_next_a), dim=1)
        ...
        u[self.agent_id] [:,-1,:]= self.actor_network(o[self.agent_id])
