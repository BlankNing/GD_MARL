...
from modules.agents.actor_critic_s import Actor,Critic
...
class MADDPG_Attention:
    ...
    def train(self, transitions, other_agents):
        ...
        # the q loss
        q_value = self.critic_network(o_s, u_s)

        critic_loss = (target_q - q_value).pow(2).mean()

        # the actor loss
        # 重新选择联合动作中当前agent的动作，其他agent的动作不变 this step can cause some problem when using transformer
        u_s[self.agent_id] = self.actor_network(o[self.agent_id])
        actor_loss = - self.critic_network(o_s, u_s).mean()
        # if self.agent_id == 0:
